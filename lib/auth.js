...
// Auth config from environment variables (fallback when users.json is empty)
...
// Migrate existing env credentials to users.json on first load
users.migrateFromEnv();
...
function login(username, password) {
  ...
  // Multi-user mode: check users.json first
...
function _writeEnv() {
  ...
  // Update or add YT_AUTH_USER and YT_AUTH_PASS, preserve everything else
  let foundUser = false, foundPass = false;
  lines = lines.map(line => {
    if (line.startsWith('YT_AUTH_USER=')) { foundUser = true; return 'YT_AUTH_USER=' + AUTH_USER; }
    if (line.startsWith('YT_AUTH_PASS=')) { foundPass = true; return 'YT_AUTH_PASS=' + AUTH_PASS; }
    return line;
  });
  if (!foundUser) lines.push('YT_AUTH_USER=' + AUTH_USER);
  if (!foundPass) lines.push('YT_AUTH_PASS=' + AUTH_PASS);
...
module.exports = { login, validate, logout, authMiddleware, updateCredentials, getUsername, sendVerificationCode, verifyCode, getSession };
