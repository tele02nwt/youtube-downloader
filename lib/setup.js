...
// Find binary in common install locations (cross-platform)
function findBinary(name) {
  const candidates = IS_WINDOWS
    ? [
        // winget default locations
        `C:\\Program Files\\Cloudflare cloudflared\\${name}${EXE}`,
        ...
        `${process.env.LOCALAPPDATA || ''}\\Microsoft\\WinGet\\Packages\\${name}\\${name}${EXE}`,
        // Scoop
        `${process.env.USERPROFILE || ''}\\scoop\\shims\\${name}${EXE}`,
        // Chocolatey
        `C:\\ProgramData\\chocolatey\\bin\\${name}${EXE}`,
        // rclone (Windows alternative for Google Drive)
        `C:\\Program Files\\rclone\\rclone${EXE}`,
        `${process.env.USERPROFILE || ''}\\scoop\\apps\\rclone\\current\\rclone${EXE}`,
      ]
    : [
  ...
  // Fallback: use where (Windows) or which (Unix)
  try {
    const cmd = IS_WINDOWS ? 'where' : 'which';
    const found = execFileSync(cmd, [`${name}${EXE}`], { encoding: 'utf8' }).trim().split('\n')[0];
...
function startGdriveAuth() {
  const bin = findBinary('gog');
  if (!bin) throw new Error('gog CLI 未安裝。請依照安裝指南「可選步驟 B」的步驟安裝 gog CLI。');
