...
// Multi-channel notifications (P4)
...
function getFallbackThumbnail(url) {
  const videoId = extractYouTubeVideoId(url);
  return videoId ? `https://i.ytimg.com/vi/${videoId}/maxresdefault.jpg` : null;
}

function resolveThumbnail({ thumbnail, url }) {
  return thumbnail || getFallbackThumbnail(url) || null;
}
...
function parseVideoInfo(info) {
  ...
  return {
    ...
    thumbnail: resolveThumbnail({ thumbnail: info.thumbnail, url: info.webpage_url || info.original_url || urlFromInfo(info) }),
...
function urlFromInfo(info) {
  if (info.webpage_url || info.original_url) return info.webpage_url || info.original_url;
...
// Start a download
function startDownload({ url, title, thumbnail, formatId, audioFormatId, resolution, categoryId, categoryName, remuxFormat, datePrefix, audioOnly, audioFormat, speedLimit, subtitles, scheduledAt, userId }) {
  const id = uuidv4();
  ...
  if (scheduledAt && new Date(scheduledAt).getTime() > Date.now()) {
    ...
    _scheduledDownloads.set(id, {
      scheduledAt,
      params: { url, title, formatId, audioFormatId, resolution, categoryId, categoryName, remuxFormat, datePrefix, audioOnly, audioFormat, speedLimit, subtitles, userId }
    });

    logger.info('download', `排程下載: ${title || 'Unknown'} → ${new Date(scheduledAt).toLocaleString('zh-TW')}`, { id, scheduledAt, userId });
    return record;
  }
  ...
  logger.info('download', `開始下載: ${title || 'Unknown'}`, {
    id, url, resolution, remuxFormat, categoryName, outputDir, userId
  });
    ...
    if (code === 0) {
      ...
      if (downloadedFile) {
        ...
        logger.info('upload', `開始上傳 Google Drive: ${title || 'Unknown'}`, {
          id, file: downloadedFile, categoryName, userId: current?.userId || null
        });
        try {
          gdriveInfo = await uploadToGdrive(downloadedFile, categoryName);
          if (gdriveInfo) {
            logger.success('upload', `Google Drive 上傳成功: ${title || 'Unknown'}`, {
              id, fileId: gdriveInfo.id, link: gdriveInfo.webViewLink, userId: current?.userId || null
            });
          } else {
            logger.warn('upload', `Google Drive 上傳失敗（無返回值）: ${title || 'Unknown'}`, { id, userId: current?.userId || null });
          }
        } catch (e) {
          console.error('Google Drive upload error:', e.message);
          logger.error('upload', `Google Drive 上傳錯誤: ${e.message}`, {
            id, title, error: e.message, userId: current?.userId || null
          });
        }
      }
      ...
      logger.success('download', `下載完成: ${completed?.title || title || 'Unknown'}`, {
        ...
        gdrive: !!gdriveInfo, gdriveLink: gdriveInfo?.webViewLink || null, userId: completed?.userId || null
      });
      // Send multi-channel notification (P4)
      notifier.sendNotification('download_complete', {
        title: completed?.title || title || 'Unknown',
        url,
        resolution: resolution || 'best',
        format: remuxFormat,
        categoryName: categoryName || '未分類',
        gdrivePath: gdriveInfo?.webViewLink || null,
        fileSize: completed?.filesize || null
      });
    } else {
      ...
      logger.error('download', `下載失敗: ${title || 'Unknown'}`, {
        id, url, resolution, error: errMsg.substring(0, 500), userId: current?.userId || null
      });
      // Send multi-channel notification (P4)
      notifier.sendNotification('download_error', {
        title: title || 'Unknown',
        url,
...
function retryDownload(id) {
  ...
  logger.info('download', `重試下載: ${original.title}`, { oldId: id, newId: newRecord.id, userId: original.userId || null });
  return newRecord;
}
...
function listDownloadsForSession(session, filters) {
  const scopedUserId = tenantAccess.getScopedUserId(session);
  if (typeof filters === 'string') {
    return listDownloads(scopedUserId ? { status: filters, userId: scopedUserId } : filters);
  }
  if (!filters) {
    return listDownloads(scopedUserId ? { userId: scopedUserId } : undefined);
  }
  return listDownloads(scopedUserId ? { ...filters, userId: scopedUserId } : filters);
}

function getQueueStatusForSession(session) {
  if (tenantAccess.isAdminSession(session)) {
    return getQueueStatus();
  }

  const downloads = listDownloadsForSession(session, {});
  const activeStatuses = new Set(['downloading', 'merging', 'uploading', 'pending']);
  const queuedStatuses = new Set(['queued']);
  return {
    active: downloads.filter(download => activeStatuses.has(download.status)).length,
    queued: downloads.filter(download => queuedStatuses.has(download.status)).length,
    maxConcurrent: MAX_CONCURRENT_DL
  };
}

function getDownloadForSession(id, session) {
  const download = getStatus(id);
  if (!download) {
    return null;
  }
  return tenantAccess.canAccessRecord(download, session) ? download : null;
}
  ...
  for (const d of downloads) {
    if (d.status === 'scheduled' && d.scheduledAt) {
      const params = {
        ...
        subtitles: d.subtitles, userId: d.userId || null
      };
...
module.exports = {
  ...
  getStatus,
  listDownloads,
  listDownloadsForSession,
  getDownloads,
  getDownloadForSession,
  deleteDownload,
  cancelSchedule,
  getQueueStatus,
  getQueueStatusForSession,
  activeProcesses,
