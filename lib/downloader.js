...
function ensureGdriveRootFolder() {
  ...
  // Create it
  const created = gogExec(['drive', 'mkdir', GDRIVE_ROOT_FOLDER_NAME, '-j']);
...
function ensureGdriveCategoryFolder(categoryName) {
  ...
  // Create subfolder
  const created = gogExec(['drive', 'mkdir', categoryName, '--parent', rootId, '-j']);
  if (created && created.id) {
    gdriveCategoryFolderIds[categoryName] = created.id;
    return created.id;
  }
...
function uploadToGdrive(localFilePath, categoryName) {
  ...
  try {
    ...
    console.log('Uploaded to Google Drive:', parsed.name || parsed.id);
    return parsed;
  } catch (e) {
